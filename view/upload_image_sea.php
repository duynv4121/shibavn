        ...
        if(in_array($filetype, $allowed)){
            ...
            if(file_exists("../upload/" . $filename)){
				...
				if($compressedImage){ // có thể có lỗi
					mysql_query("UPDATE `ns_listhoadon_sea` SET `img`='$newFileName' WHERE (`id`='$idbill')") or die(mysql_error());
				}else{
...
<body>
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <div style="text-align:center;" class="container-fluid">
            <?php 
                $laythongtinbill = mysql_fetch_assoc(mysql_query("SELECT * FROM ns_listhoadon_sea WHERE `id`='$idbill'"));
