...
?>
<div class="container-fluid"><a href="m_admin.php?m=services_price<?php echo '&banggia='.$_GET['banggia'];?>" class="btn btn-primary btn-sm"><i class="fas fa-step-backward"></i> BẢNG GIÁ TỔNG </a><br><br>
...
<div class="card-body">
				...
				<div class="form-group" >
					...
					$laythongtingiadichvua = mysqli_query($conn,"select * from ".$danhsach_banggia." where id_dichvu='$id_dichvu' AND note='$note' ");
...
{
	echo'			<div class="col-md-5" style="color:green">Cập nhật mức giá <br><br><br>
';
...
$text = nl2br($text); // insert <br /> before \n 
	...
	mysqli_query($conn,"UPDATE `".$danhsach_banggia."` SET `$bang_gia`='$macj' WHERE (`id_dichvu`='$id_dichvu') AND (`m_price` LIKE '$mabill') AND (`note`='$note')")or die(mysqli_error());

	echo 'Cập nhật '.$string_banggia.' Mức kg :<b> '.$mabill.'-'.$id_dichvu.'-'.$note;
...
echo'</div>';
			...
			<table id="example2" class="display nowrap cell-border dataTable no-footer dtr-column collapsed" width=100% data-page-length='50'  style="font-size:12px" data-order='[[0, "desc"]]'>
		            ...
		            <tbody>
		               ...
		               $data = mysqli_query($conn,"SELECT * FROM ".$danhsach_banggia." where id_dichvu='$id_dichvu' AND note='$note'");
