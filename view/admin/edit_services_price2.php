...
?>
<div class="container-fluid"><a href="m_admin.php?m=services_price2" class="btn btn-primary btn-sm"><i class="fas fa-step-backward"></i> BẢNG GIÁ TỔNG </a><br><br>
...
<div class="card-body">
				...
				<div class="form-group" >
					...
					$laythongtingiadichvua = mysqli_query($conn,"select * from ksn_giadichvu2 where id_dichvu='$id_dichvu' AND note='$note' ");
...
$text = nl2br($text); // insert <br /> before \n 
	...
	mysqli_query($conn,"UPDATE `ksn_giadichvu2` SET `$bang_gia`='$macj' WHERE (`id_dichvu`='$id_dichvu') AND (`m_price`='$mabill') AND (`note`='$note')")or die(mysqli_error());
...
echo'</div>';
			...
			<table id="example2" class="display nowrap cell-border dataTable no-footer dtr-column collapsed" width=100% data-page-length='50'  style="font-size:12px" data-order='[[0, "desc"]]'>
		            ...
		            <tbody>
		               ...
		               $data = mysqli_query($conn,"SELECT * FROM ksn_giadichvu2 where id_dichvu='$id_dichvu' AND note='$note'");
