      ...
      <?php  
         echo '
            <div class="col-xs-3 mb-3 mr-2">
               <a href="scanin_partner.php" class="btn btn-secondary ">
                  Scan in
               </a>
            </div>

            <div class="col-xs-3 mb-3">
               <a href="scangiao_partner.php" class="btn btn-warning ">
                  Scan delivery
               </a>
            </div>
         ';
         ...
         <table id="example2" class="table table-hover table-bordered table-striped" data-page-length='50'  data-order='[[0, "asc"]]'>
            ...
            <tbody>
               ...
               if ($roleid == 1 or $roleid == 2) {
                   $data = mysql_query("SELECT * FROM ns_shipment ");
               }else{
                  $data = mysql_query("SELECT * FROM ns_shipment WHERE id_doitac='$uid'");
               }
               ...
               {  
                  ...
                  $totalcannang = mysql_fetch_assoc(mysql_query("SELECT SUM(cannang) as w FROM ns_listhoadon WHERE awb ='".$item['awb']."'"));
                  $i++;
                  echo '<tr>
                  ...
                  echo'<a href="viewbillinshipment.php?id='.$item['id'].'" type="button" class="btn btn-info"><i class="far fa-eye"></i></a>
                  </td>
                  </tr>';
               }
               ?>
