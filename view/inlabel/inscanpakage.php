...
<?php
  ...
  $nguoigui = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_nguoigui WHERE id='".$data['id_nguoigui']."'"));
  $nguoinhan = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_nguoinhan WHERE id='".$data['id_nguoinhan']."'"));
  $tp = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM yn_province WHERE id='".$nguoigui['province_id']."'"));
  $quan = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM yn_district WHERE id='".$nguoigui['district_id']."'"));
  $phuong = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM yn_ward WHERE id='".$nguoigui['ward_id']."'"));
...
<table border=0 cellpadding=0 cellspacing=0 width=563 style='border-collapse:
 ...
 <tr height=88 style='mso-height-source:userset;height:66.0pt'>
  ...
  height:60px'><img width=285 height=60 src="../../inbill/inbilltw/logo.jpg"  style="object-fit: contain;"
  alt=""
...
<script>
  ...
  function onReady()
  {
   var qrcode = new QRCode("id_qrcode", {
    text:"https://giaphuexpress.com/view/trackingview.php?id="+document.getElementById('abcId').value,
    width:80,
    height:80,
    colorDark:"#000000",
    colorLight:"#ffffff",
