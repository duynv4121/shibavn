<?php  
	...
	$payment = mysql_query("SELECT * FROM ns_payment WHERE id_package = '$id_package'");
	...
	$total = mysql_fetch_assoc(mysql_query("SELECT * FROM ns_package WHERE id = '$id_package'"));
	...
	if (isset($_POST['btn_submit'])) {
		...
		if(isset($_FILES["photo"]) && $_FILES["photo"]["error"] == 0){
			...
			$newFileName = 'receipt_air_'.$ganthoigian.'.'. pathinfo($_FILES["photo"]["name"] ,PATHINFO_EXTENSION); 
		...
		if (round($tong_thanhtoan) != 0  AND ((round($tienmat) + round($chuyenkhoan)) == $conlai)) {
			mysql_query("UPDATE `ns_package` SET `payment` = 1 WHERE id ='$id_package'") or die(mysql_error());
