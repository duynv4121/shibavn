      ...
      <div class="col-md-12">
         ...
         <table id="example2" class="table table-hover table-bordered table-striped" data-page-length='100'  >
            ...
            <tbody>
               ...
               <?php
               $data = mysql_query("SELECT * FROM ns_listhoadon WHERE id_package = '$getId'") or die(mysql_error());
               ...
               {  
                  ...
                  $count = mysql_num_rows(mysql_query("SELECT id FROM ns_listhoadon WHERE id_package = '".$package['id']."'"));
                  ...
                  <td>
                     ...
                     <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalInShippingmark" data-whatever="'.$item['id'].'"><i class="fa fa-print"></i></button>

                     <a href="edit_full_sub_package.php?id='.$item['id'].'" type="button" class="btn btn-warning"><i class="far fa-edit"></i></a>
                     <a href="trackingview.php?id='.$item['id'].'" type="button" class="btn btn-info"><i class="fas fa-search-location"></i></a>
                  </td>
                  <td><a href="edit_full_package.php?id='.$package['id'].'" type="button" class="btn btn-warning"><i class="far fa-edit"></i></a> Date: '.$package['date'].' - ID: '.$package['id'].' - Người gửi: '.$sName['name'].' - Người nhận: '.$rName['name'].' - Số kiện: '.$package['sokien'].'(nhận khách) '.$count.' (thực tế) - Total: '.$package['total'].'</td>
                  </tr>';
               }
               ?>
              ...
              api
                  ...
                  .each(function (group, i) {
                      if (last !== group) {
                          $(rows)
                              .eq(i)
                              .before('<tr style="background-color: #b8d0de;" class="group"><td colspan="5"><b>' + group + '</b> </td></tr>');
