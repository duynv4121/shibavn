            ...
            <div class="delivery-status-5Tf">
			  ...
			  if($laydulieuhoadon['status'] >= 1)
				...
				if($laydulieuhoadon['status'] >= 2 && $laydulieuhoadon['status'] < 5)
				{
					...
					$laytime33 = mysqli_fetch_assoc(mysqli_query($conn,"select date,status,address from ns_tracking_bill where id_hoadon='$idhoadon' AND date < '$datenow' order by date DESC"));
