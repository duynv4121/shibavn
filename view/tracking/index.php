...
$id = $_GET['b'];
		...
		if(strlen($id) == 10)
					...
					else
					{
						...
						$arr = json_decode($jsonb,true);
						$package = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_package WHERE id ='".$laydulieuhoadon['id_package']."'"));
						$cuscode = mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM ns_customer WHERE cus_code ='".$package['cus_code']."'"));
						$sName = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_nguoigui WHERE id ='".$package['id_nguoigui']."'"));
						$rName = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_nguoinhan WHERE id ='".$package['id_nguoinhan']."'"));
						@$dulieuquocgia = mysqli_fetch_assoc(mysqli_query($conn,"select name from ns_countries where id='".$rName['country_id']."'"));
						$laydichvu = mysqli_fetch_assoc(mysqli_query($conn,"select dichvu,discount from ksn_dichvu where id='".$package['kg_dichvu']."'"));
						$laydulieuhoadonb = mysqli_query($conn,"select * from ns_listhoadon where id_package='".$package['id']."'");
  ...
  <nav class="main-header navbar navbar-expand-md navbar-light navbar-white" style="background-color:#FFFFFF">
    <div class="container">
      <a href="" class="navbar-brand">
        <span class="brand-text font-weight-light"><img src="logotscpost.png" width=150px height=60px></span>
      </a>
      ...
      <ul class="order-1 order-md-3 navbar-nav navbar-no-expand ml-auto">
        ...
        <li class="nav-item">
          ...
          <div class="input-group input-group-sm">
            <input class="form-control form-control-navbar" type="trackingcode" name="b" placeholder="Nhập mã Tracking" aria-label="Search" height="48" size="50" style="color:red;background-color:white;border:2px solid blue">
        ...
        <div class="row">
			    ...
			    <div class="col-lg-4">
					<div class="card card-primary">
					  <div class="card-header" style="background-color:#eb2630">
				  ...
				  $package = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_package WHERE id ='".$laydulieuhoadon['id_package']."'"))or die ("Nhập chính xác mã vận đơn của bạn để kiểm tra tracking");
                  $cuscode = mysqli_fetch_assoc(mysqli_query($conn,"SELECT id FROM ns_customer WHERE cus_code ='".$package['cus_code']."'"))or die ("loi 3");
                  $sName = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_nguoigui WHERE id ='".$package['id_nguoigui']."'"));
                  $rName = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_nguoinhan WHERE id ='".$package['id_nguoinhan']."'"));
				  @$dulieuquocgia = mysqli_fetch_assoc(mysqli_query($conn,"select name from ns_countries where id='".$rName['country_id']."'"));
				  $laydichvu = mysqli_fetch_assoc(mysqli_query($conn,"select dichvu,discount from ksn_dichvu where id='".$package['kg_dichvu']."'"))or die(mysql_error());
					$countries = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_countries WHERE id = '".$rName['country_id']."'"));
						$citynhan = mysqli_fetch_assoc(mysqli_query($conn,"SELECT name FROM cities WHERE id = '".$rName['city']."'"));
					  ...
					  echo'
						...
						<p class="text-muted" style="font-weight:bold">
						  ...
						  <span class="tag tag-success">ĐỊA CHỈ: '.($rName['address']).' '.$citynhan['name'].'</span><br>
						  <span class="tag tag-success">QUỐC GIA: '.($countries['name']).' </span><br>
						  <span class="tag tag-success">POST CODE: '.($rName['post_code']).' </span><br>
						...
						<h3 class="card-title">Lịch trình di chuyển</h3>
                                        ...
                                        <div class="card-body">
                                            <div class="vertical-timeline vertical-timeline--animate vertical-timeline--one-column">
												...
												$test = 0;
												@$laydulieutrackb = mysqli_query($conn,"select id from ns_tracking_bill where id_hoadon='$idhoadon' AND (`status`='DESTINATION CUSTOMS RELEASED' OR `status`='Deliver the package to FWD' OR 'status'='ON FORWARD FOR DELIVERY') AND date < '$datenow' LIMIT 1");
												...
												foreach(@$arr as $result) 
													...
													if(strpos($result['status'], "Đã tạo nhãn cho kiện hàng") !== false || strpos($result['status'], "Kiện hàng đã được nhận") !== false|| strpos($result['status'], "On forward for delivery") !== false|| strpos($result['status'], "created a label") !== false ){}
													else
													{
