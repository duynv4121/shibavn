<?php  
	...
	function congno($idbill)
	{
		if($idbill == 1)
		{
			$status_string = '<font color=red style="">DAY</font color>';
		}else if($idbill == 2)
		{
			$status_string = '<font color=blue>WEEK</font color>';
		}else if($idbill == 3)
		{
			$status_string = '<font color=green>MONTH</font color>';
		}else if($idbill == 4)
		{
			$status_string = '<font color=blue>2 WEEK</font color>';
		}
		else
		{
	...
	function add_debit($uid,$idkhachhang,$vat_fee,$conn)
	{
		...
		$ma_debit = 'DEBIT_'.$idkhachhang.'_'.	date("Ymdhi");
		mysqli_query($conn,"INSERT INTO `ksn_debit` (`datetime`, `uid`, `idkhachhang`, `debitno`,`vat`) VALUES (now(), '$uid', '$idkhachhang', '$ma_debit','$vat_fee')");
	...
	function add_trackingBill($id_code,$status,$detail,$conn)
	{
		...
		mysqli_query($conn,"INSERT INTO `ns_tracking_bill` (`id_hoadon`,`status`, `address`,`date` ) VALUES ('$id_code', '$status', '$detail',now())") or die(mysqli_error());
	}
	...
	function createBillForPacker($uid,$id,$date,$datenow2,$cannang,$sokien,$status, $note, $pcs,$cuscode,$pack_type,$pack_length,$pack_width,$pack_height,$convert_weight,$charge_weight,$conn){
		$layid = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_listhoadon ORDER BY id DESC LIMIT 1")) or die("Loi");
		$id_code = ($layid['id_code'])+(rand(6,12));
	...
	function createPackageforSale($id_nguoigui,$cus_code,$id_nguoinhan,$uid,$sokien,$date,$chiho,$kg_dichvu,$kg_chinhanh,$kg_ref,$kg_reiceiversign,$congno,$conn,$id_sale,$khach_cannang,$khach_cuocbay,$khach_phuthu,$khach_cuocnoidia,$khach_thuho,$khach_baohiem,$khach_phibaohiem,$checkthanhtoan,$payment_type,$vat){
		$layid = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_package ORDER BY id DESC LIMIT 1")) or die("Loi");
		$layid_new = ($layid['id_code']+1);
	...
	function updatecataloglist($id_bill,$catalogid){
		$insert_query = mysql_query("INSERT INTO ns_mapcatalog (`id_bill`,`id_catalog`) VALUES ('$id_bill', '$catalogid')");
	}
