...
echo'<script> 
				...
				if(isset($_GET['b']))
				{
					...
					window.location = "home.php";
						...
						$laydichvu = mysqli_fetch_assoc(mysqli_query($conn,"select dichvu from ksn_dichvu where id='".$package['kg_dichvu']."'"))or die(mysql_error());
      ...
      <div class="frame-7150-Me9">
        <div class="value-K5B">
          <div class="auto-group-vgqf-S9o">
            ...
            <div class="delivery-status-5Tf">
			  ...
			  if($laydulieuhoadon['status'] >= 1)
				...
				if($laydulieuhoadon['status'] >= 2 && $laydulieuhoadon['status'] < 5)
				{
					...
					@$last_status	= $laytime3['status'];
					@$last_address	= $laytime3['address'];
					@$last_date	= $laytime3['date'];
			...
			if(@$arr['data'][0]['origin_info']['trackinfo'][0]['Details'] != "" && $ketnoitrack == 1)
			{
          ...
          <div class="sub-link-eQH">
            <div class="nhn-cp-nht-trng-thi-LH7"><?php echo $last_status.' ';?> <i class="fas fa-check-circle"></i></div>
          </div>
        </div>
			...
			if($laydulieuhoadon['status'] >= 2  && $laydulieuhoadon['status'] < 5)
			{
				...
				//$laytime3 = mysqli_fetch_assoc($laydulieutrackawba);
					...
					echo'<div class="value-Huj">
						<div class="delivery-status--dTo">'.@$laytime3['status'].'</div>
						<div class="inglewood-ca-us-ATj">'.@$laytime3['address'].'</div>
						<div class="auto-group-fnfs-tuX">'.@$laytime3['date'].'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>
					  </div>';
					  					$check_tree = 4;
				...
				else if($ketnoitrack == 1)
				{
					...
					if(strpos(@$arr['data'][0]['origin_info']['trackinfo'][0]['StatusDescription'],'created a label') == true)
					{
							echo'<div class="value-Huj">
							<div class="delivery-status--dTo">'.@$laytime3['status'].'</div>
							<div class="inglewood-ca-us-ATj">'.@$laytime3['address'].'</div>
					...
					{
						echo'<div class="value-Huj">
							<div class="delivery-status--dTo">'.@$arr['data'][0]['origin_info']['trackinfo'][0]['StatusDescription'].'</div>
            ...
            <div class="frame-7168-n6M">
              ...
              <div class="frame-7164-gb3">
				...
				while($laydulieupackagea = mysqli_fetch_array($laydulieuhoadonb,MYSQLI_ASSOC))
				{
					...
					echo '&nbsp;&nbsp;'.$stt.'. ID: <a href="index.php?b='.$laydulieupackagea['id_code'].'">'.$laydulieupackagea['id_code'].' </a>'; if($laydulieupackagea['billketnoi'] != ""){echo'- '.$laydulieupackagea['billketnoi'].'';}echo' </font><br>';
					$stt++;
				}
...
<div class="card-body" style="background-color:#EEEEEE;width:100%;">
                                            <div class="vertical-timeline vertical-timeline--animate vertical-timeline--one-column"  >
												...
												//Ket noi API 
                                                    ...
                                                    <div>
                                                        <span class="vertical-timeline-element-icon bounce-in">
															...
															{
																echo'<i class="badge badge-dot badge-dot-xl badge-success"> </i>';
															}else if($result['substatus'] == "transit001")
															{
																echo'<i class="badge badge-dot badge-dot-xl badge-warning"> </i>';
															}else if($result['substatus'] == "pickup001")
															{
																echo'<i class="badge badge-dot badge-dot-xl badge-primary"> </i>';
															}
															else
															{
															echo'<i class="badge badge-dot badge-dot-xl badge-secondary"> </i>';
															}
												...
												?>
												<?php
												$laydulieutrackb = mysqli_query($conn,"select * from ns_tracking_bill where `check`='1' AND id_hoadon='$idhoadon' order by id desc");
												while($laydulieutrack = mysqli_fetch_array($laydulieutrackb,MYSQLI_ASSOC))
												{
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="vertical-timeline-element-content bounce-in">
                                                            <h4 class="timeline-title" style="color:red">'.xulychuoi($laydulieutrack['status']).'</h4>
												...
												<?php
												$laydulieutracka = mysqli_query($conn,"select * from ns_tracking_bill where id_hoadon='$idhoadon' AND `check` is NULL order by id desc");
												$test = 0;
												...
												if(mysqli_num_rows($laydulieutrackb) >= 1)
												{
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="vertical-timeline-element-content bounce-in">
                                                            ...
                                                            <span class="vertical-timeline-element-date">'.$dulieuups['date'].'</span>
                                                        </div>
                                                    </div>
														</div>';
												}
