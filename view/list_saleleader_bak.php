...
while($check_teammember = mysqli_fetch_array($check_teammembera))
{
	...
	echo'	<tr>
		<td>'.$laydulieumember['ten'].'</td><td>'.$laydulieumember['cus_code'].'</td><td  style="color:green;font-weight:bold">'.number_format($loinhuancanhan).' đ</td>
          ...
          <table class="table 	projects border" style="width:100%">
              ...
              <tbody>';
					...
					else
					{
				  	...
				  	$laydulieusale = mysqli_query($conn,"SELECT * FROM ns_user where idsale_service<>'0' AND idsale_service NOT IN(select id_member from ksn_sale_leader_detail) GROUP BY idsale_service")or die("Loi");
					}
					$counta = 0;
							while ($item = mysqli_fetch_array($laydulieusale,MYSQLI_ASSOC)) {
								$counta++;
...
while($laydulieulienketfwda = mysqli_fetch_array($laydulieulienketfwd))
{
	...
	echo'	<tr>
		...
		<td style="color:red">'.number_format(debitfwd($laydulieulienketfwda['id'],$new_month,$new_year,$conn)).'đ</td>
      ...
      <!-- /.card -->
				...
				if(@$_GET['action'] == "price_fwd_sale")
				{
						...
						echo'<div class="col-md-8">';
							...
							echo'<center>BẢNG GIÁ ÁP GIÁ CODE FWD CHO SALE (<a href="list_saleleader.php?action=fwdmanager">DANH SÁCH SALE HỖ TRỢ</a>)<br>Nếu không có trong danh sách mặc định (<font color=red><i>Cost Leader:5,000đ - Cost Member:3,000đ - Cost Leader Member:2,000đ</i>)</font></center>';
