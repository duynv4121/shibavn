        ...
        if(is_uploaded_file($_FILES['file']['tmp_name'])){
            ...
            while(($line = fgetcsv($csvFile)) !== FALSE){
				...
				if($demsokien == $itemCount)
				{
					...
					if($linkpdf1 != "")
					{
						            echo '<font color=green>Tạo Label '.$referenceNo.' thành công !</font><br>';

					}
					else
					{
						            echo '<font color=red>Tạo Label '.$referenceNo.' không thành công !</font><br>';
					}
...
<div class="row">
   ...
   <center> <div class="col-md-12" id="importFrm" style="background-color:#EEEEEE; padding:10px;border:1px solid gray" >
		...
		<a href="labelau.php?updatelabel=1">Label AUPOSTE</a>
    </div> </center>
...
<table id="example3" class="display nowrap " style="width:100%">
            ...
            <tbody>
               ...
               if ($roleid == 1 || $roleid == 3 || $roleid == 4 || $roleid == 5) {
				   ...
				   {
					$data = mysqli_query($conn,"SELECT * FROM ksn_labelau where (id_name<>'') order by id DESC LIMIT 1000");
