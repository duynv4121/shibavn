...
$sheet->setCellValue('E1','CÔNG TY TNHH THƯƠNG MẠI VÀ DỊCH VỤ SHIBA EXPRESS')->getStyle('E1')->applyFromArray($styleArray);
...
while($row  = mysqli_fetch_array($result,MYSQLI_ASSOC))
{
	...
	{
		$checkstring = $listhoadonphuthu['tenphuthu'];
	@$pos = strpos($stringphuthu, $checkstring);
	if($pos !== false)
	{
		$stringphuthu.= '';
	}
	...
	if($dulieukhachhang['payment_price_type_c'] != '' AND $dateimport >= $dulieukhachhang['payment_price_type_date'])
	{
			if($dulieukhachhang['payment_price_type_c'] == 1)
			{
				if($result2a['kg_chinhanh'] == "HCM")
				{
					$bang_gia = 'price_hcm_f1'; 
			...
			else if($dulieukhachhang['payment_price_type_c'] == 0)
			{
				if($result2a['kg_chinhanh'] == "HCM")
				{
					$bang_gia = 'price'; 
	...
	### Tesst app gia
	@$dulieuapgia = mysqli_fetch_assoc(mysqli_query($conn,"select * from ksn_price_package where id_package='".$result2a['id_code']."'"));
	$check_apgia = 0;
	if(@$dulieuapgia['price_discount'] > 0)
	{
		$sheet->setCellValue('Q'.$rowCount,number_format($dulieuapgia['price_discount']*$total_weight+$phuthukien))->getStyle('O'.$rowCount)->applyFromArray($styletotal);
						$sheet->setCellValue('S'.$rowCount,'Áp giá : '.$dulieuapgia['price_discount'].'/kg');

		$totalprice += $dulieuapgia['price_discount']*$total_weight;		
		$check_price = 3;
		$check_apgia = 1;
		$giakienhangnew = $dulieuapgia['price_discount']*$total_weight;
	}
	else
	{
		...
		//* Tinh Gia DICH VU KSN-CANADA,KSN-UK,ksn-US,KSN-US NDA,KSN-SIN,KSN-AUE  |||| US-KM,USF-KM,KSN-US3, KSN-DUBAI
		else if($result2a['kg_dichvu'] == 13 || $result2a['kg_dichvu'] == 34 || $result2a['kg_dichvu'] == 14 || $result2a['kg_dichvu'] == 7|| $result2a['kg_dichvu'] == 9|| $result2a['kg_dichvu'] == 10 || $result2a['kg_dichvu'] == 11 || $result2a['kg_dichvu'] == 28|| $result2a['kg_dichvu'] == 25 || $result2a['kg_dichvu'] == 30 || $result2a['kg_dichvu'] == 31|| $result2a['kg_dichvu'] == 32|| $result2a['kg_dichvu'] == 33)
		{
		...
		else if($result2a['kg_dichvu'] == 41)
		{
			...
			$check_price = 1;
				...
				echo $note;
			@$dulieudiscount = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ksn_discount WHERE id_dichvu='".$result2a['kg_dichvu']."' AND uid='".$dulieukhachhang['id']."' AND (DATE(date_start) <='".$dateimport."') AND (DATE(date_end) >='".$dateimport."') AND `note` ='".$note."' LIMIT 1"));
			echo $dulieudiscount['id'];
			
			if($dulieudiscount['id'] == "")
			{
				echo'123';
			@$dulieudiscount = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ksn_discount WHERE id_dichvu='".$result2a['kg_dichvu']."' AND uid='".$dulieukhachhang['id']."' AND (DATE(date_start) <='".$dateimport."') AND (DATE(date_end) >='".$dateimport."') LIMIT 1"));
			}
			
						echo @$dulieudiscount['d_m_10'];
			...
			if($check_apgia == 0)
			{
			if($dulieudiscount['id_dichvu'] != "")
			{
				echo'123123';
			echo @$dulieudiscount['d_m_10'];
				
			if($total_weight >= 10 and $total_weight < 21)
			{
				$discount = @$dulieudiscount['d_m_10'];
				$total_discount += $discount;
				if($discount > 0)
				{
				$sheet->setCellValue('S'.$rowCount,'Discount: -'.number_format($discount));
				}
			}else if($total_weight >= 21 and $total_weight < 45)
			{
			...
			else if($total_weight >= 45 && $total_weight < 71)
			{
			...
			}else if($total_weight >= 71 && $total_weight < 100)
			{
				$discount = @$dulieudiscount['d_m_71']*$total_weight ;
				$total_discount += $discount;
				if($discount > 0)
				{
				$sheet->setCellValue('S'.$rowCount,'Discount: -'.number_format($discount));
			...
			if($dulieudebit['vat'] == 1)
			{
				...
				}else if($check_price == 3)
				{
					$totalvat_kien = ($giakienhangnew)*8/100;
					$sheet->setCellValue('Q'.$rowCount,number_format($giakienhangnew+$phuthukien+$totalvat_kien));
					$sheet->setCellValue('N'.$rowCount,($totalvat_kien));
...
$sheet->setCellValue('A'.($rowCount),"1/ Quý khách vui lòng kiểm tra và xác nhận đề nghị thanh toán (Debit note) ngay khi quý đại lí nhận được và gửi lại biên lai cho SHIBA EXPRESS trong ngày.")->getStyle('A'.($rowCount))->applyFromArray($styleArray1);
$rowCount+=1;

$objExcel->getActiveSheet()->mergeCells('A'.($rowCount).':R'.($rowCount));
$sheet->setCellValue('A'.($rowCount),"   Bởi vì quý đại lí thuộc công nợ ngày của SHIBA EXPRESS, vì vậy vui lòng kiểm tra và xác nhận debit thanh toán càng sớm càng tốt trong ngày, để tránh chậm trễ tiến độ xuất hàng trong ngày. ")->getStyle('A'.($rowCount))->applyFromArray($styleArray1);
$rowCount+=1;
...
$objExcel->getActiveSheet()->mergeCells('A'.($rowCount).':R'.($rowCount));
$sheet->setCellValue('A'.($rowCount),"3/ Nếu quá 7 ngày chúng tôi vẫn chưa nhận được thanh toán, SHIBA EXPRESS sẽ phát hành khoản phí thanh toán muộn là 2% cho tổng giá trị hóa đơn, và khoản phí này sẽ được charge vào tài khoản của quý khách hàng. ")->getStyle('A'.($rowCount))->applyFromArray($styleArray1);
$rowCount+=1;
}
