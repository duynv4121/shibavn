...
if(isset($_POST['valid_button']))
{
	...
	{
		mysqli_query($conn,"UPDATE `ns_package` SET `checkthanhtoan`='2' WHERE (`id_code`='".$id_bill."')");
		mysqli_query($conn,"UPDATE `ns_user` SET `hanmuc`=`hanmuc`+$totalcuoc WHERE (`id`='$user_id')");
		mysqli_query($conn,"UPDATE `ksn_debit_sale` SET `valid`='1', `valid_uid`='$uid', `final_price`='$totalcuoc', `valid_time`='$datenow' WHERE (`id`='$id_debit')");
	}
	else
	{
		mysqli_query($conn,"UPDATE `ns_package` SET `checkthanhtoan`='2' WHERE (`id_code`='".$id_bill."')");
		...
		<table id="example3" class="display nowrap cell-border" style="width:100%">
            ...
            <tbody>
               ...
               {  
                  ...
                  $package = mysqli_fetch_assoc(mysqli_query($conn,"SELECT * FROM ns_package WHERE id_code ='".$item['id_bill']."'"));
				 ...
				 <input type="hidden" name="user_id" value="'.$package['id_sale'].'">
                  ...
                  <td style="text-align: center; color:black;font-size:14px;">'.$package['payment_type'].'</td>
