...
<?php
	...
	while($laydulieukien = mysqli_fetch_array($laydulieukienadd))
	{
...
<table border=0 cellpadding=0 cellspacing=0 width=474 style=\'border-collapse:
 ...
 <tr height=19 style=\'height:14.4pt\'>
  ...
  if($thongtinuser['logo'] != "")
  {
	  ...
	  echo'<img width=167 height=86 src='.$thongtinuser['logo'].' v:shapes="Picture_x0020_2" style="object-fit: contain;"></span><![endif]><span
 ...
 <tr height=21 style="height:15.6pt">
  ...
  <td class=xl65></td>
  <td class=xl65></td>
  <td class=xl65 colspan=4 style="mso-ignore:colspan">TEL:
  '.$thongtinuser['phone'].'</td>
 </tr>
 <tr height=19 style="height:14.4pt">
  <td height=19 class=xl65 style="height:14.4pt"></td>
  <td class=xl65></td>
  <td class=xl65></td>
  <td class=xl65></td>
  <td class=xl65 colspan=3 style="mso-ignore:colspan">Service:<b> '.$dulieudichvu['dichvu'].'</b></td>
  <td class=xl65></td>
 </tr>
 ';
  }
  ...
  <table cellpadding=0 cellspacing=0>
   ...
   <tr>
	...
	$quocgia_name = mysqli_fetch_assoc(mysqli_query($conn,"select * from ns_countries where id='$quocgia'"));
		...
		$string7 = 'Freight :';
		$string8 = 'VAS ：';
		$string9 = 'Total Charge :';

		echo'<center>'.@$quocgia_name['iso2'].'</td>';
...
';
	...
	$laydulieukienaddz = mysqli_query($conn,"select * from ns_listhoadon where id_package='".$id."'");
		...
		while($laydulieukienzz = mysqli_fetch_array($laydulieukienaddz))
		{
	echo'
	var qrcode = new QRCode("id_qrcode'.$laydulieukienzz['id'].'", {
				text:"http://giaphuexpress.com/view/tracking/index1.php?id="+document.getElementById(\'abcId'.$laydulieukienzz['id'].'\').value,
				width:120,
				height:115,
				colorDark:"#000000",
				colorLight:"#ffffff",
